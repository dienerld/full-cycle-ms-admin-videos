...
export class SearchParams<Filter = string> extends ValueObject {
  ...
  private setPage(value: number) {
    ...
    if (Number.isNaN(_page) || _page <= 0 || Number.parseInt(_page as never) !== _page) {
      _page = 1
    }
  ...
  private setPageSize(value: number) {
    ...
    if (
      ...
      _pageSize <= 0 ||
      Number.parseInt(_pageSize as never) !== _pageSize
    ) {
  ...
  private setSortBy(value: string | null) {
    this._sortBy = value === null || value === undefined || value === '' ? null : `${value}`
  }
